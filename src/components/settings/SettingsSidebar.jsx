...
import { User, PlayCircle, Folder, Database, ShieldCheck } from 'lucide-react';
...
export default function SettingsSidebar({ activeTab, onSelectTab }) {
  return (
    <nav className="w-64 shrink-0 flex flex-col gap-1 select-none">
      <div className="px-3 pb-3 mb-1 border-b border-[#EFECE6]">
        <h1 className="text-xl font-black text-[#212529] tracking-tight">การตั้งค่า</h1>
        <p className="text-[11px] text-[#8C857B] mt-0.5">จัดการระบบ TubeLock Studio</p>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3.5 px-4 py-3 rounded-xl text-xs font-semibold transition-all text-left cursor-pointer ${
                isActive
                  ? 'bg-[#FFF2E5] text-[#FF7A00] font-bold shadow-2xs border-l-3 border-[#FF7A00]'
                  : 'text-[#3A352F] hover:bg-[#EFECE6]'
              }`}
            >
              <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-[#FF7A00]' : 'text-[#8C857B]'}`} />
              <div className="flex flex-col min-w-0">
                <span className="truncate leading-none">{tab.label}</span>
                <span className={`text-[10px] mt-1 font-normal truncate ${isActive ? 'text-[#D96800]' : 'text-[#8C857B]'}`}>
                  {tab.desc}
                </span>
              </div>
            </button>
